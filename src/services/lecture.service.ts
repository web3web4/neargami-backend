...
for (let i=0;i<lectures.length;i++){
  ...
  let title= lectures[i].title;
   lectures[i].slug= await this.stringToSlugById(title,id);
   let sluge=lectures[i].slug;
   const updatedLectures: Lecture=await this.lecture.update({where:{id},
    data:{
slug:sluge
    }  
  })
...
async findAllLEcturesBySlugAuth(slug:string,user_id:string){
  ...
  return await this.lecture.findFirst({where:{AND:{slug}},include:{
...
///
  ...
  public async findAllWithIdAuthServic(course_id: number, user_id: string): Promise<any> {
    ...
    // Calculate total completed lessons
    const completedLessons = completedUserLecture.reduce(
      (total, lecture) => total + lecture._count.end_at,
      0
    );
